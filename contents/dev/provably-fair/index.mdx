...
**Provably Fair**(증명 가능한 공정성)는 이 문제를 암호학으로 해결한 알고리즘입니다. 운영자와 사용자가 서로를 믿을 필요 없이, 결과가 공정하게 만들어졌음을 수학적으로 검증할 수 있습니다.
...
커밋먼트 스킴은 암호학에서 **값을 미리 약속(commit)하고 나중에 공개(reveal)하는 프로토콜**입니다. 핵심은 두 가지 성질입니다.

- **은닉성(Hiding)**: 해시만으로는 원본을 알 수 없음
- **결합성(Binding)**: 나중에 다른 값으로 바꿔치기할 수 없음

SHA-256 같은 암호학적 해시 함수는 이 두 성질을 모두 만족합니다. 해시를 먼저 공개한 이상 운영자는 게임 도중 서버 시드를 바꿀 수 없습니다. 나중에 원본 시드를 공개했을 때 해시가 일치하는지 확인하면 그만입니다.
