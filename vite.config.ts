...
import { remarkDiscordTimestamp } from "./src/plugins/remark-discord-timestamp";
...
export default defineConfig(({ command, mode }): UserConfig => {
  return {
    plugins: [tailwindcss(), qwikRouter({
      mdx: {
    ...
    }), qwikVite(), viteStaticCopy(
      {
        ...
        targets: [
          {
            src: 'contents/**/*.{png,jpg,jpeg,webp,svg,avif,gif}',
            dest: '.',
            rename: (fileName, fileExtension, fullPath) => {
              return path.relative('contents', fullPath);
            }
          }
        ],
      }
